...
export const Sidebar = () => {
  ...
  const handleMenuClick = (item: typeof menuItems[0]) => {
    if (item.path === "#") {
      toast({
        title: `${item.label}`,
        description: `${item.label} functionality coming soon...`,
      });
            ...
            <li key={index}>
              {item.path === "#" ? (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 text-left text-slate-300 hover:bg-slate-700 hover:text-white"
                >
